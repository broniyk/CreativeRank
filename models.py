...
def get_pooled_dataset(
    data_df: pd.DataFrame,
    cols: List[str],
    cat_cols: List[str],
    pos_neg_ratio: float = None,
):
    if pos_neg_ratio is not None:
        ...
        data_df_neg = data_df[data_df["CLICK"] == 0][
            ["EXPERIMENT_ID", "RECIPIENT_ID"]
        ].sample(frac=pos_sz / neg_sz * pos_neg_ratio, random_state=42)
        pos_neg_df = pd.concat([data_df_pos, data_df_neg]).set_index(
            ["EXPERIMENT_ID", "RECIPIENT_ID"]
        )
        data_df = (
            data_df.set_index(["EXPERIMENT_ID", "RECIPIENT_ID"])
            .loc[pos_neg_df.index]
            .reset_index()
        )
